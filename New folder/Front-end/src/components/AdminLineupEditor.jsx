...
export const AdminLineupEditor = ({ players, selectedMatch, onNavigate, onStatsSaved }) => {
  ...
  if (!selectedMatch) {
    return (
      <div className="lineup-page">
        ...
        <button onClick={() => onNavigate('admin-matches')}>
          Go to Matches
        </button>
      </div>
    );
  ...
  // Players not already in this match's lineup
  // so you can't add the same player twice
  ...
  const handleAddPlayer = async (e) => {
    ...
    try {
      ...
      // RecalculatePlayerStatsAsync
      await matchPlayersApi.addToMatch(
        selectedMatch.id,
        selectedPlayerId,
        {
          position,
      ...
      // Refresh this match's lineup list
      loadLineup();

      // Tell App.jsx to re-fetch players
      // so updated stats show everywhere
      onStatsSaved();
    } catch (err) {
  ...
  const handleRemove = async (playerId) => {
    ...
    try {
      ...
      // Stats need to drop back down too
      // since RecalculatePlayerStatsAsync runs on removal too
      onStatsSaved();
    } catch (err) {
  ...
  return (
    <div className="lineup-page">
      <button
        className="lineup-back-btn"
        onClick={() => onNavigate('admin-matches')}
      >
        ← Back to Matches
      </button>
      ...
      <div className="lineup-columns">
        ...
        <div>
          <h2 className="lineup-section-title">
          ...
          <div className="lineup-current-list">
            {lineup.map((entry) => (
              <div
                key={entry.playerId}
                className="lineup-current-row"
              >
                <div>
                  <p className="lineup-current-row-name">
                    {entry.playerName}
                  </p>

                  <p className="lineup-current-row-meta">
                    {entry.position} • {entry.tries} tries •{' '}
